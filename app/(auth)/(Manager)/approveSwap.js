    ...
    useEffect(() => {
        const fetchApprovedSwaps = async () => {
            try {
                ...
                const approvedSwapsRef = ref(db, 'approved-actions');

                onValue(approvedSwapsRef, (snapshot) => {
                const data = snapshot.val();
                if (data) {
                    const fetchedApprovedSwaps = [];
                    Object.keys(data).forEach((swapKey) => {
                        const swap = data[swapKey];
                        fetchedApprovedSwaps.push({ key: swapKey, ...swap });
                    });
                    setApprovedSwaps(fetchedApprovedSwaps);
                } else {
                setApprovedSwaps([]);
            }
        });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <View style={styles.swapContainer}>
          <Text style={styles.headerText}>Approved Swaps</Text>
          {approvedSwaps.map((swap, index) => (
            <View key={index} style={styles.approvedSwapItem}>
